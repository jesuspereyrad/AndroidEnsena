package com.example.jesuspereyra.test.training;
...
public class TrainingActivity extends AppCompatActivity implements CvCameraViewListener2 {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        textToSpeech = new TextToSpeech(TrainingActivity.this, new TextToSpeech.OnInitListener() {
            @Override
...
//        builder.set
                                    ...
                                    public void onClick(DialogInterface dialog, int id) {
                                        server2 = new SendToServer(new ResultCallback() {
                                            ...
                                            public void success(String results) {
                                                Toast.makeText(TrainingActivity.this, "Enviando al servidor", Toast.LENGTH_SHORT).show();
                                            }
                                        });
                                        server2.execute(buffer, IP_ADDRESS, "5679", input.getText().toString());
                                        stop();
                                    }
                                });
                        builder2.create();
    ...
    public void TextMode(Mat mRgba) {
        Imgproc.threshold(GRAY, WB, 58, 255, Imgproc.THRESH_BINARY_INV);
        Imgproc.Canny(mRgba, ROI, 100, 255);
        Core.add(WB, ROI, ROI2);
        ROI3 = ROI2.submat(roi);
        Imgproc.cvtColor(ROI3, ROI3, Imgproc.COLOR_GRAY2RGBA);
        ROI3.copyTo(mRgba.submat(new Rect(10 ,10 ,ROI3.cols(), ROI3.rows())));
    }
            ...
            public void success(String results) {
                if(results != null) {
                    if(!results.toLowerCase().matches("no hay mano")) {
                        Toast.makeText(TrainingActivity.this, results, Toast.LENGTH_SHORT).show();
                        textToSpeech.speak(results, TextToSpeech.QUEUE_FLUSH, null);
//                           alert(results, buffer);
                        stop();
                    }
                } else {
                    Toast.makeText(TrainingActivity.this, "Bad", Toast.LENGTH_SHORT).show();
...
//        Every 25 frames enter to the condition and save the image in the android device
